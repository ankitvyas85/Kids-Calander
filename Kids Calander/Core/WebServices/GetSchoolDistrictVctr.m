...
@implementation GetSchoolDistrictVctr
@synthesize vctr = _vctr;
@synthesize arrForResult = _arrForResult;
...
- (void)didFinishFetchingRecords:(id)response{
    if([self.vctr respondsToSelector:@selector(SuccessfullyReceivedSchoolresponse:)]) {
        id <GetSchoolWebServiceDelegate> obj = (id)self.vctr;
        [obj SuccessfullyReceivedSchoolresponse:response];
	}
}
- (void)didFailWithError:(id)sender{
    if([self.vctr respondsToSelector:@selector(FailWithError:)]) {
        id <GetSchoolWebServiceDelegate> obj = (id)self.vctr;
        [obj FailWithError:sender];
	}
}
