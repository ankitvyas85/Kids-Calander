...
@implementation GetStandardVctr
...
- (void)didFinishFetchingRecords:(id)response{
    if([self.vctr respondsToSelector:@selector(SuccessfullyReceivedStandardresponse:)]) {
        id <GetStandardDelegate> obj = (id)self.vctr;
        [obj SuccessfullyReceivedStandardresponse:response];
	}
}
