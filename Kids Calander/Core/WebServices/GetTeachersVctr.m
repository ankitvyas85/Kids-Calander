...
@implementation GetTeachersVctr
...
- (void)didFinishFetchingRecords:(id)response{
    if([self.vctr respondsToSelector:@selector(SuccessfullyReceivedTeacherListresponse:)]) {
        id <GetTeachersListDelegate> obj = (id)self.vctr;
        [obj SuccessfullyReceivedTeacherListresponse:response];
	}
}
