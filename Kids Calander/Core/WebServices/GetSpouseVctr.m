...
- (void)didFinishFetchingRecords:(id)response{
    if([self.vctr respondsToSelector:@selector(SuccessfullyReceivedSpouseresponse:)]) {
        id <GetSpouseDelegate> obj = (id)self.vctr;
        [obj SuccessfullyReceivedSpouseresponse:response];
	}
}
- (void)didFailWithError:(id)sender{
    if([self.vctr respondsToSelector:@selector(SpouseFailWithError:)]) {
        id <GetSpouseDelegate> obj = (id)self.vctr;
        [obj SpouseFailWithError:sender];
	}
}
