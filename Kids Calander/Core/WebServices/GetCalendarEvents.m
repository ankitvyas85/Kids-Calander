...
- (void)didFinishFetchingRecords:(id)response{
    if([self.vctr respondsToSelector:@selector(SuccessfullyReceivedEventsListresponse:)]) {
        id <GetEventsListDelegate> obj = (id)self.vctr;
        [obj SuccessfullyReceivedEventsListresponse:response];
	}
}
