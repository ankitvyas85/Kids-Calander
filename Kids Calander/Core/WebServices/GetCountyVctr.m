...
- (void)didFinishFetchingRecords:(id)response{
    if([self.vctr respondsToSelector:@selector(SuccessfullyReceivedresponse:)]) {
        id <WebServiceDelegate> obj = (id)self.vctr;
        [obj SuccessfullyReceivedresponse:response];
	}
}
